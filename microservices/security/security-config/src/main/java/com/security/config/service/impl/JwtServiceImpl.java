...
public class JwtServiceImpl implements JwtService {
    ...
    public String generateAccessToken(UserDto userDto) {
        ...
        return Jwts.builder().id(UUID.randomUUID().toString()).subject(userDto.getName().toString()).issuer(issuer)
                ...
                .claims(Map.of("email", userDto.getEmail(), "roles", userDto.getRoles().stream().map(RolesDTO::getName).toList(), "tokenType", "access"))
    ...
    public String generateRefereshToken(UserDto userDto, String jwtId) {
        ...
        return Jwts.builder().id(jwtId).subject(userDto.getName().toString()).issuer(issuer)
                .expiration(Date.from(now.plusSeconds(refreshTtlSeconds))).claims(Map.of("email", userDto.getEmail(), "roles", userDto.getRoles().stream().map(RolesDTO::getName).toList(), "tokenType", "refresh"))
                .signWith(secretKey, SignatureAlgorithm.HS512).compact();
    }
